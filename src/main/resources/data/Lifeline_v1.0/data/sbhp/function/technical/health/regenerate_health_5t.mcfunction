...
# Multiply TimeToHealBase by 4, move into TimeToHeal
scoreboard players set @s sbhp.math.A 4
...
#region
## Initialize Math A Score to 1000, representing 100.0% of healing being applied
scoreboard players set @s sbhp.math.A 1000
...
scoreboard players set @s[predicate=sbhp:player/running,nbt=!{active_effects:[{id:"minecraft:nausea"}]}] sbhp.math.B 70
...
scoreboard players set @s[predicate=sbhp:player/running,nbt=!{active_effects:[{id:"minecraft:nausea"}]}] sbhp.math.B 100
scoreboard players operation @s[predicate=sbhp:player/running,nbt=!{active_effects:[{id:"minecraft:nausea"}]}] sbhp.math.A /= @s sbhp.math.B

### If Nausea effect is active, sprinting reduces healing by 50%
scoreboard players set @s[predicate=sbhp:player/running,nbt={active_effects:[{id:"minecraft:nausea"}]}] sbhp.math.B 50
scoreboard players operation @s[predicate=sbhp:player/running,nbt={active_effects:[{id:"minecraft:nausea"}]}] sbhp.math.A *= @s sbhp.math.B
scoreboard players set @s[predicate=sbhp:player/running,nbt={active_effects:[{id:"minecraft:nausea"}]}] sbhp.math.B 100
scoreboard players operation @s[predicate=sbhp:player/running,nbt={active_effects:[{id:"minecraft:nausea"}]}] sbhp.math.A /= @s sbhp.math.B
...
### Nether reduces Healing by 15% overall
scoreboard players set @s[predicate=sbhp:location/nether] sbhp.math.B 85
...
scoreboard players set @s sbhp.math.B 0
execute unless score @s sbhp.DmgTimer <= $config sbhp.config.DmgWait store result score @s sbhp.math.B run data get entity @s equipment.chest.components."minecraft:enchantments"."sbhp:recovery" 8
scoreboard players operation @s sbhp.math.A += @s sbhp.math.B
...
#region
## Multiply Math A variable by Visual Max Health and then divide by 20000 (Sets to 0.5% of max health and then adds or subtracts what is needed in the same breath)
## 40000 is used in this case because healing takes place 4x per second, meaning each second you'd heal 2% of health by default
##tellraw @s[scores={sbhp.math.A=1..}] [{"text":"Heal Percentage: ","color":"white"},{"score":{"name":"@s","objective":sbhp.math.A}}]
scoreboard players operation @s sbhp.math.A *= @s sbhp.HealthMaxBase
scoreboard players set @s sbhp.math.B 200000
scoreboard players operation @s sbhp.math.A /= @s sbhp.math.B
...
##tellraw @s[scores={sbhp.math.A=1..}] [{"text":"Heal Amount: ","color":"white"},{"score":{"name":"@s","objective":sbhp.math.A}}]
scoreboard players operation @s sbhp.math.A += @s sbhp.math.C

### Regeneration Effect is an *absolute increase* (meaning it cannot be modified) of +10 Health Regen per level
...
execute as @s[scores={sbhp.math.C=0..}] run scoreboard players set @s sbhp.math.B 125
