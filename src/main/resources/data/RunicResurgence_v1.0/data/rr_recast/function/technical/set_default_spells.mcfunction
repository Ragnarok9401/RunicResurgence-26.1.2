...
#   - Cooldown determines how long it takes for any spells to become available after a cast has completed
...
#       - Squared Change float, determines how the Change is modified over time (if at all)
...
#
#   = Payload field denotes how the projectile's effects carry out on activation, requires:
#       - Max Targets int; denotes max number of entity targets the spell can target (or infinite if -1)
#       - AOE float; denotes how far the effects take place centered on the spell's position
#       - Trigger can be Any (block, entity, periodic, or expiration), Collision (only Block or Entity), Block, Entity, Timed (periodic, expiration), Periodic, Expiration, or Constant
#      (for persistent effects along a path)
#       - Entity Trigger Range denotes the radius for which the spell will search for entities to trigger the payload
#       - Block Trigger Range denotes the radius for which the spell will search for blocks to trigger the payload
#       - Base Damage float; the damage that the spell will do
#       - Damage Type str; the type of damage dealt by the spell, not necessarily always the same as the Spell's type (for instance, can be rr_aea:magic_reverb type)
#
...
#   -   4) Projectile Count denotes how many projectiles spawn, each receive their own unique Spread.
...
# Magic Bolt
data modify storage rr_recast:global spells.magic_bolt set value \
{\
    name:"Magic Bolt",\
    reference:magic_bolt,\
    windup:60,\
    cooldown:40,\
    type:combat,\
    subtype:attack,\
    ...
    windup_fx:{\
        reference:magic_bolt,\
        sound_1:"minecraft:block.amethyst_block.break player @a[distance=..6,tag=!rr.spell.silenced] ~ ~1 ~ 0.15 0.1 0",\
        sound_1_timing:"10..",\
        sound_2:"minecraft:entity.blaze.ambient master @a[distance=..40] ~ ~1 ~ 0.1 1.75 0",\
        sound_2_timing:"10..",\
        particle_1:"portal ~ ~1 ~ 0 0 0 0.8 2 normal @a[distance=..18,tag=!rr.spell.noparticles]",\
        particle_1_timing:"5..",\
    },\
    vector:\
    {\
    ...
    {\
        ...
        projectile_id:0,\
        trigger:any,\
        entity_trigger_range:2.0f,\
        block_trigger_range:0.0f,\
        explosion_aoe:3.0f,\
        speed:0.5f,\
        acceleration:0.0f,\
        speed_max:2.5f,\
        ...
        damage:4.0f,\
        projectile_count:1,\
        projectile_count_delay:0,\
        damage_type:'rr_recast:magic_generic',\
        homing:false,\
        damage_randomness:0.0f,\
        lifespan_randomness:0,\
        angle_randomness:0.0f,\
        speed_randomness:0.0f,\
        summon_sound:"block.enchantment_table.use player @a[distance=..6,tag=!rr.spell.silenced] ~ ~1 ~ 0.75 1.5 0",\
        trail:"reverse_portal ~ ~ ~ 0.05 0.05 0.05 0.01 1 normal @a[distance=..30,tag=!rr.spell.noparticles]",\
        explosion_sound:"entity.lightning_bolt.impact player @a[distance=..30] ~ ~ ~ 0.4 1.33 0",\
        explosion_particle_1:"explosion ~ ~ ~ 0 0 0 0 1 force @a[distance=..30]",\
        explosion_particle_2:"witch ~ ~ ~ 1.5 1.5 1.5 0.08 20 normal @a[distance=..30,tag=!rr.spell.noparticles]",\
        split:\
        {\
            reference:magic_bolt,\
            max_splits:1,\
        ...
        {\
            item_display:"head",\
            CustomName:[{"text":"Arcane Bolt","color":"light_purple","italic":false}],\
            CustomNameVisible:false,\
            teleport_duration:1,\
            Tags:\
            [\
                "rr.projectile.magic_bolt",\
                "rr.projectile",\
                "rr.entity",\
                "rr.setup.magic_bolt",\
                "rr.spell.ticking"\
            ...
            {\
                ...
                components:\
                {\
                    "item_model":"rr_recast:spell/generic/bolt",\
                    "dyed_color":\
                    [\
                        0.875,\
                        0.35,\
...
data modify storage rr_recast:global spells.paladin_pylon set value \
{\
    name:"Paladin's Pylon",\
    reference:paladin_pylon,\
    windup:120,\
    cooldown:220,\
    minimum_windup:40,\
    minimum_cooldown:100,\
    type:combat,\
    subtype:defense,\
    delivery:aura,\
    element:arcane,\
    ...
    {\
        ...
        {\
            ...
            particle:\
            {\
                particle:"dust{scale:0.8,color:[0.25,0.65,0.65]} ~ ~0.125 ~ 1.5 0 1.5 0.01 4 normal @a[distance=..30]",\
                id:"minecraft:dust{\"scale\":0.8,\"color\":[0.25,0.65,0.65]}",\
                amount:4,\
                distance:[3.5f,0.0f,3.5f],\
