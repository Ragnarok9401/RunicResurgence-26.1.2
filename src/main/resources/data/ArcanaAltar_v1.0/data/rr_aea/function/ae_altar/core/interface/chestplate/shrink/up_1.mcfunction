...
execute as @n[type=marker,limit=1,sort=nearest,tag=ArenAEADTMain,distance=..5] if score @s aea.math.F matches 4 run data modify entity @n[type=item_display,tag=ArenAEAShrinkLvl,distance=..5] item.components.minecraft:item_model set value "aea:icon/level_4"

# Replace current interaction with a new one to refresh the Interaction timestamps
summon interaction ~ ~ ~ {width:0.25f,height:0.25f,response:true,Tags:[aea.altar.entity,"ArenAEAShrinkUp1","ArenAEAIMain"]}
kill @s