...
# Adding a level costs 40 levels (only 1 level, so only 1 possible value)
execute as @s at @s if score @s aea.system.level_base matches 0 if score @s aea.system.level_difference matches 1 run scoreboard players set @s aea.math.H 25
