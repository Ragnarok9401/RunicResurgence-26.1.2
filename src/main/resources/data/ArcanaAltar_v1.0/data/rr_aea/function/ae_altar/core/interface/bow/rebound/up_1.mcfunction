...
execute as @n[type=marker,limit=1,sort=nearest,tag=ArenAEADTMain,distance=..5] if score @s aea.math.F = @s aea.math.E run title @p[distance=..15] actionbar {"text":"Enchantment may not go above level 2","color":"red","italic":false}
...
execute as @n[type=marker,limit=1,sort=nearest,tag=ArenAEADTMain,distance=..5] if score @s aea.math.F matches 1 run data modify entity @e[type=item_display,tag=ArenAEAReboundLvl,limit=1,sort=nearest] item.components.minecraft:item_model set value "aea:icon/level_1"
execute as @n[type=marker,limit=1,sort=nearest,tag=ArenAEADTMain,distance=..5] if score @s aea.math.F matches 2 run data modify entity @e[type=item_display,tag=ArenAEAReboundLvl,limit=1,sort=nearest] item.components.minecraft:item_model set value "aea:icon/level_2"

# Replace current interaction with a new one to refresh the Interaction timestamps
summon interaction ~ ~ ~ {width:0.25f,height:0.25f,response:true,Tags:[aea.altar.entity,"ArenAEAReboundUp1","ArenAEAIMain"]}
kill @s