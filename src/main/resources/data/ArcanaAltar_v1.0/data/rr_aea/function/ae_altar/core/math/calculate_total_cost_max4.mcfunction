...
# Adding a level costs 12 * (target level - current level) + (3 * current level)
execute as @s at @s if score @s aea.system.level_base matches 0 if score @s aea.system.level_difference matches 1 run scoreboard players set @s aea.math.H 8
