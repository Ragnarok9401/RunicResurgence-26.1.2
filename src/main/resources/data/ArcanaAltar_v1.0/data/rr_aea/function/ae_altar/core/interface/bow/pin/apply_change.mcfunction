...
execute as @n[tag=ArenApplyEnch,type=marker,distance=..15] run data remove entity @n[type=item_display,tag=ArenAEAIDMain,tag=ArenAEAIDMStart] item.components.minecraft:enchantments."rr_aea:pin"
execute as @n[tag=ArenApplyEnch,type=marker,distance=..15] if score @s aea.math.F matches 1 run data modify entity @n[type=item_display,tag=ArenAEAIDMain,tag=ArenAEAIDMStart] item.components.minecraft:enchantments."rr_aea:pin" set value 1
...
execute as @n[type=item_display,tag=ArenAEAltar] at @s run function rr_aea:ae_altar/core/back/bow/bow_ench_pin