...
### Regeneration Effect is an *absolute increase* (meaning it cannot be modified) of 1.66% of Max Health per level
...
execute as @s[scores={sbhp.math.C=0..}] run scoreboard players set @s sbhp.math.B 1666
...
#region
## Multiply Math A variable by Visual Max Health and then divide by 200000 (Sets to 0.5% of max health and then adds or subtracts what is needed in the same breath)
...
scoreboard players set @s sbhp.math.B 2000000
scoreboard players operation @s sbhp.math.A /= @s sbhp.math.B
