...
execute as @s[tag=rr.player.sneaking_consecutive] run return fail
...
execute as @a[predicate=rr_recast:player_checks/sneaking,predicate=rr_recast:held_items/mainhand/tablet/any_runic_tablet,tag=!rr.spell.casting,tag=!rr.spell.tablet_active,tag=!rr.player.sneaking_consecutive,tag=rr.player.sneaking_initial] at @s run function rr_recast:spells/runic_ring/create_runic_ring