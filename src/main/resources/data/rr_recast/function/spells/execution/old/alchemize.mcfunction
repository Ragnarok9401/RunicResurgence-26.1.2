...
execute as @s[tag=RunicAlchemize,tag=RunicSpellRun] at @s if data entity @s equipment.offhand unless predicate rr_recast:held_items/offhand/alchemizable run title @s actionbar {"text":"You cannot alchemize this!","color":"red","italic":false}
execute as @s[tag=RunicAlchemize,tag=RunicSpellRun] at @s unless data entity @s equipment.offhand run title @s actionbar {"text":"You must hold an item in your Offhand to alchemize!","color":"red","italic":false}

# Handle general alchemizable items
execute as @s[tag=RunicAlchemize,tag=RunicSpellRun] at @s if data entity @s equipment.offhand if predicate rr_recast:held_items/offhand/alchemizable unless data entity @s Inventory[{Slot:-106b,id:"minecraft:gold_nugget"}] run function rr_recast:technical/spell_extra/alchemize_item_count
