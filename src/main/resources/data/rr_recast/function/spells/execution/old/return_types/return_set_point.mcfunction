...
execute as @s[tag=RunicReturnCheck,tag=RunicSpellRun,tag=!RunicReturnSet,scores={rr.math.NumTrack1=1..}] at @s unless entity @e[type=marker,tag=RunicReturnPoint,tag=!RunicReturnAlign,distance=..2] run execute as @n[type=marker,tag=RunicReturnPoint] at @s run forceload add ~ ~ ~ ~
